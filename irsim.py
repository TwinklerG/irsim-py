...
# Internal filename: source_code/irsim.py
# Bytecode version: 3.8.0rc1+ (3413)
# Source timestamp: 2023-02-02 03:07:45 UTC (1675307265)
...
class IRSim(QMainWindow, ui_mainwindow.Ui_MainWindow):
    ...
    curRowSignal = pyqtSignal()
    stopSignal = pyqtSignal()
    stepSignal = pyqtSignal()
    aboutSignal = pyqtSignal()
    codeListSignal = pyqtSignal(int)
    ...
    def __init__(self, parent=None):
        ...
        self.codeList.clicked.connect(self.updateRowLabel)
    ...
    def fileOpen(self):
        ...
        fname = QFileDialog.getOpenFileName(
            self, "IR Simulator - Open IR file", dir, "IR files (*.ir)"
        )
    ...
    def run(self):
        ...
        while not (self.ip < 0 or self.ip >= len(self.codes)):
    ...
    def updateStatus(self, message):
        ...
        if self.filename is None:
            ...
            self.setWindowTitle(
                "IR Simulator - {0}".format(os.path.basename(self.filename))
            )
        ...
        try:
            if strs[0] == "LABEL" or strs[0] == "FUNCTION":
                ...
                if strs[0] == "GOTO":
                    ...
                    if len(strs) != 2:
                        raise IRSyntaxError
                    if (strs[0] == "READ" or strs[0] == "PARAM") and (
                        not strs[1][0].isalpha()
                    ):
                        raise IRSyntaxError
    ...
    def labelCheck(self):
        try:
            ...
            QMessageBox.critical(
                self, "Error", "Undefined label at line %d:\n\n%s" % (i + 1, code)
            )
            return False
        return True
    ...
    def displayWatchTable(self):
        for row, key in enumerate(self.symTable.keys()):
            ...
            if self.symTable[key][0] < 0:
                ...
                strs = str(
                    self.mem[
                        self.symTable[key][0] // 4 : self.symTable[key][0] // 4
                        + self.symTable[key][1] // 4
                    ]
                )
    ...
    def execute_code(self, code):
        ...
        try:
            if code[0] == "READ":
                ...
                self.mem[self.symTable[code[1]][0] // 4] = eval(
                    str(value1) + code[3] + str(value2)
                )
            elif code[0] == "RETURN":
                ...
                for key in self.functionDict[code[2]]:
                    ...
                    self.symTable[key] = (
                        self.getNewAddr(self.symTable[key][1]),
                        self.symTable[key][1],
                        self.symTable[key][2],
                    )
...
if args.input_file is not None:
    irsim_cli = IRSimCli()
    irsim_cli.loadFile(args.input_file)
    irsim_cli.run()
else:
